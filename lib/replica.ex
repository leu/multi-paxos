...
#Setters
defp leaders(self, v) do Map.put(self, :leaders, v) end
defp requests(self, v) do Map.put(self, :requests, v) end
defp decisions(self, v) do Map.put(self, :decisions, v) end
defp proposals(self, v) do Map.put(self, :proposals, v) end
defp slot_out(self, v) do Map.put(self, :slot_out, v) end
defp slot_in(self, v) do Map.put(self, :slot_in, v) end
...
defp next(self) do
  self = receive do
    ...
    {:decision, s, c} ->
      self = self |> decisions([{s, c}] ++ self.decisions)
      perform_all(self)
  end
  self = propose(self)
  next(self)
...
defp perform_all(self) do
  ...
  if tuple_size(ready_slot) == 2 do
    ...
    self = if tuple_size(proposal) == 2 do
      # IO.inspect(self.proposals, label: "self.proposals")
      # IO.inspect(proposal, label: "proposal")
      self = self |> proposals(self.proposals -- [proposal])
...
defp get_slot(s, list) do
  Enum.reduce(list, {},
    fn {s_h, c_h}, exists -> if s_h == s do {s_h, c_h} else exists end end)
end

defp propose(self) do
  if self.slot_in < self.slot_out + self.config.window_size
      and length(self.requests) > 0 do
    [c | requests_tail] = self.requests
    self = if !slot_exists?(self.slot_in, self.decisions) do
      self = self |> requests(requests_tail)
      self = self |> proposals([{self.slot_in, c}] ++ self.proposals)
...
defp slot_exists?(slot_in, decisions) do
  Enum.reduce(decisions, false,
    fn {s, _}, exists -> if s == slot_in do true else exists end end
  )
end
...
defp c_is_previous(slot_out, cmd, decisions) do
  Enum.reduce(decisions, false,
    fn {s_h, cmd_h}, exists -> if cmd_h == cmd and s_h < slot_out do true else exists end end
  )
end
